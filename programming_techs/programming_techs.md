...
### Проблематика
В современной индустрии существует огромное количество платформ (платформа=архитектуры*ОС).  
Каждая платформа накладывает свои особенности сборки, к примеру:
- уникальные опции оптимизации на конкретном CPU;
- включение дополнительных опций на определенных платформах;
- использование разных компиляторов;
- release/debug опции и т.д.

---

![bg right:100% 95%](images/additionals/build_tools/gcc_build_options.png)

---

Возрастает сложность. Нужна унификация. Инженеры для борьбы с этой сложностью придумывают системы и новые абстракции.  
  
Разрабатывают **системы сборки** (системы автоматизации сборки, build tools, BT).
...
Типичные действия, выполняемые BT:
- скачивание зависимостей;
- компиляция;
- линковка;
- тестирование;
- пакетизация;
- подготовка дополнительных пакетов (например документации).

---

#### msbuild

Стандартный BT в Visual Studio. Инструкции для сборки пишутся в `.vsxproj` файлах в xml формате. Настройки билда в VS всего лишь визуализация этого файла.  
  
Первый и последний плюс msbuild заключается в бесшовной интеграции с VS.

---

![bg right:100% 95%](images/additionals/build_tools/vcxproj.png)

---

#### CMake

Кроссплатформенный универсальный генератор сборочных файлов. К примеру используя cmake можно сгенерировать `.vsxproj` файлы. Синтаксис языка намного лаконичней и читабельней, чем у msbuild. Инструкции для сборки пишутся в `CMakeLists.txt` файлах.
...
- **хорошая** поддержка множества платформ и BT;
