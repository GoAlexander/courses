...
3) Перед архивацией произвести фильтрацию списка файлов: архивируем N самых старых файлов (в зависимости от даты модификации), чтобы выполнить условие с `X`-порогом.

---

4) Написать 2-ой shell-скрипт, который будет генерировать тест-кейсы и проверять корректность работы базового скрипта (фактически написать тесты), как минимум 4 теста. Во всех тест-кейсах папка `/log` должна весить минимум 0,5 GB. Тестовый скрипт должен полностью подготавливать окружение для тестирования, то есть всё должно быть автоматизировано, никаких ручных подготовительных действий. Скрипт работающий в user-space (без использования root-прав), будет оцениваться выше.

5) Вывод скриптов должен быть понятен и информативен, но без лишней белиберды. Поставьте себя на место пользователя.

---

* **на 9 баллов**: если выставлена переменная окружения `LAB1_MAX_COMPRESSION=1`, то сжатие производится, используя алгоритм `lzma` (достаточно воспользоваться ключом для `tar`). Написать отдельный тест-кейс на этот случай. Справочно: [тыц1](https://www.reddit.com/r/learnprogramming/comments/t69bxt/bz2_gzip_or_lzma_which_do_you_prefer/), [тыц2](https://www.opennet.ru/opennews/art.shtml?num=20958).
* **на 10 баллов**: в дополнение к `.sh` написать все тоже самое, только для ОС Windows (`.bat`)

---

**Регламент защиты лабораторной работы:**
- Заранее прислать по email ссылку на проект в github/gitlab, проект должен быть открыт для просмотра
- На защите:
    - не больше 1 минуты - демонстрация работы скриптов (с точки зрения пользователя)
    - 2 минуты - показать код и самостоятельно прокомментировать наиболее важные объекты
    - ответить на вопросы
- Студенты должны уметь ответить на любой вопрос в рамках лабораторной работы. Для ответа можно пользоваться консолью.
